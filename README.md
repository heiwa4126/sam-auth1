...
テストのためにToken Sourceをデフォルトの"Authorization"から変えてある。
...
OutputのHelloWorldApiのURLに対してcurlで
Auth:ヘッダに `allow`で認証される。
...
```bash
sam-auth1$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-auth1$ sam local start-api
sam-auth1$ curl http://localhost:3000/
```
...
```bash
sam-auth1$ sam logs -n HelloWorldFunction --stack-name sam-auth1 --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-auth1
```
